...
from keras.layers import TimeDistributed, Flatten, concatenate, Bidirectional
...
dataset_4 = pd.read_csv('./inputFiles/downsampled_drug_export.csv')
...
dataset_y_sbpFinal = pd.read_csv('./outcomeFiles/downsampled_finalsbp_outcome.csv')
...
#y = (y == 1)
#
#y = ((hba1c_final_y >= 48) & (hba1c_final_y <= 60) & (sbp_final_y <= 140))
#y = (y < (-10))
#
#y = hba1c_final_y
#y = sbp_final_y
...
#y_test = yScaler.transform(y_test)finalhba1c_outcome
...
for loopn in range(1, numberOfLoops):
    ...
    lstm_out = Bidirectional(LSTM(return_sequences=False, input_shape = (len(set1[0]), LSTMinputDim), units=LSTM_layer1, dropout = lstm_dropout_1, recurrent_dropout = lstm_dropout_2))(merged)
    ...
    adam = optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1, decay=0.0)
